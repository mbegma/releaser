...
def _create_files_object_list(source_dir: Path, extension_list: list, current_dir: Path, destination_dir: Path) -> list:
    """
    ...
    :param current_dir: Текущая директория, откуда необходимо скопировать файлы
    ...
    for extension in extension_list:
        ...
        _count = 0
        # находим разницу между списками parts у объектов current_dir и path_obj и это и будет неизменная часть,
        # ее добавляем к destination_dir и получаем полный путь, куда нужно скопировать файл
        for path_obj in _files_generator:
            _diff_list = [x for x in list(path_obj.parts) if x not in list(current_dir.parts)]
...
def main():
    ...
    print(f"Директория '{destination_dir}' успешно создана или уже существует.")

    _out_tmp_dir = tempfile.TemporaryDirectory(dir=destination_dir)
    print(f"tmp dir: {_out_tmp_dir.name}")

    dest_list = _create_files_object_list(source_dir, args.extensions, current_dir, Path(_out_tmp_dir.name))
    _copy_files_count = _copy_files_object_list(dest_list)
    print(f"copied {_copy_files_count} files from {len(dest_list)} to {_out_tmp_dir.name}.")

    source_directory = Path(_out_tmp_dir.name)
    output_zip_file = destination_dir / f"{current_dir.name}_{datetime.now():%Y-%m-%d_%H_%M_%S}.zip"
    if _zip_directory(source_directory, output_zip_file):
        print(f"zipped {output_zip_file}")
    else:
        print(f"failed to zip {output_zip_file}")
